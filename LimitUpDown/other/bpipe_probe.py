...
MIN_LIMIT and MAX_LIMIT for a name like 7203 JT Equity, and do they agree
with the static PX_MAX_LIMIT / PX_MIN_LIMIT pair - before anyone builds a
feed around the answer.
...
THREE PROBES, in increasing order of what they assume:
  ...
  fieldinfo   //blp/apiflds    Do MIN_LIMIT and MAX_LIMIT exist, and what
                               do they hold?  It does NOT say whether a
                               field is real-time or static - only a
                               FieldSearchRequest filtered on fieldType
                               does, which is bpipe_fields.py.
  snapshot    //blp/refdata    The limits now, next to the static
                               PX_MIN_LIMIT and PX_MAX_LIMIT pair.
                               Run 2026-09-03 against a Japanese name, the
                               PX_ fields came back "Field not permitted to
                               datafeed users" while MIN_LIMIT and MAX_LIMIT
                               answered.  They are not two names for one
                               thing: PX_ is the static reference family, and
                               a B-PIPE datafeed entitlement does not carry
                               it.  Anything the new job needs must be found
                               in the real-time family.
...
#  The snapshot asks for more: the same two, the static pair beside them,
#  and a last price to sanity check that the limits bracket it.
#  PX_LAST is in here next to LAST_PRICE for the same reason PX_MIN_LIMIT is
#  next to MIN_LIMIT: to show, in the output, which family this entitlement
#  actually serves.  LAST_PRICE is the one that answers, and it is what makes
#  the "is the last price inside the band" check work at all.
REF_FIELDS = ["MIN_LIMIT", "MAX_LIMIT", "LAST_PRICE",
              "PX_MIN_LIMIT", "PX_MAX_LIMIT", "PX_LAST",
              "CRNCY", "NAME"]
...
def probe_fieldinfo(session, identity, fields) -> bool:
    ...
    #  Keyed on the MNEMONIC, never on the id the response carries: Bloomberg
    #  answers with its own field id (PR005 and the like), so keying on that
    #  makes every field we asked about look missing.
    found = set()
    for msg in _send(session, request, identity):
        ...
        for i in range(data.numValues()):
            ...
            if entry.hasElement("fieldError"):
                print(f"  {field_id:<16} NOT A FIELD  "
                      f"{entry.getElement('fieldError')}")
                continue
            info = entry.getElement("fieldInfo")

            def text(name):
                return (info.getElementAsString(name)
                        if info.hasElement(name) else "")

            mnemonic = text("mnemonic") or field_id
            found.add(mnemonic.upper())
            print(f"  {mnemonic:<16} {text('ftype'):<10} "
                  f"{text('datatype'):<10} {text('description')}")

    missing = [f for f in fields if f.upper() not in found]
    if missing:
        ...
        return False
    #  ftype above is a data category (Price, Character), NOT real-time vs
    #  static.  FieldInfoRequest cannot tell you which family a field is in;
    #  only a FieldSearchRequest filtered on fieldType can.  That is what
    #  bpipe_fields.py is for.
    return True


# =============================================================================
# PROBE 2 - the limits right now, next to the static pair
# =============================================================================
...
def probe_snapshot(session, identity, security: str, fields) -> bool:
    ...
    #  LAST_PRICE first: PX_LAST is barred to a datafeed entitlement, so
    #  reading it alone leaves the check permanently inert.
    last = to_decimal(values.get("LAST_PRICE")) or to_decimal(
        values.get("PX_LAST"))
    ...
    if (low, high) != (None, None) and (r_low, r_high) != (None, None):
        if (low, high) == (r_low, r_high):
            print("  MIN/MAX_LIMIT agree with PX_MIN/PX_MAX_LIMIT exactly - "
                  "the real-time pair carries the same numbers")
        else:
